...
void Addon::adjustColorTemperature(uint32_t *px, int n, bool rgbw) {
    ...
    if (t <= 66.0f) {
        rF = 1.0f;
        gF = 0.3900815788f * std::log(t) - 0.6318414438f;
        bF = (t <= 19.0f) ? 0.0f : (0.5432067891f * std::log(t - 10.0f) - 1.1962540891f);
    } else {
        rF = 1.2929361861f * std::pow(t - 60.0f, -0.1332047592f);
        gF = 1.1298908609f * std::pow(t - 60.0f, -0.0755148492f);
        bF = 1.0f;
    }
    ...
    // stripType
    {
        ...
        if (maybe_stripType.ToLocal(&stripType)) {
            ...
            if (stripTypeValue == "rgb")
                ...
                config.ws281x.channel[0].strip_type = WS2811_STRIP_RBG;
                ...
                config.ws281x.channel[0].strip_type = SK6812_STRIP_RBGW;
