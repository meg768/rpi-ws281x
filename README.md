...
| transitions   | *(string, experimental)* Built-in pixel transitions. **"warm-white"** - Converts RGB values to warm colors. **"white-shift" **or **"RGBtoRGBW"**  - RGB input values (packed as `0x00RRGGBB`) are converted to WRGB (packed as `0xWWRRGGBB`) by extracting the white channel using min(R,G,B). **"monochrome"** - Discards colors. So a valid value would be **"warm-white RGBtoRGBW"** to convert RGB values with warm white colors to a RGBW strip. Or just use **"RGBtoRGBW"** to convert your own RGB-strip colors to a RGBW-strip. |
